...
SMODS.Joker { --Dozer {Doesn't do anything}
    key = "dozer",
    config = {},
    pos = { x = 0, y = 0 },
    rarity = 1,
...
SMODS.Joker{ --Litany {Doesn't do anything}
    key = "litany",
    config = {},
    pos = { x = 0, y = 0 },
    rarity = 3,
...
SMODS.Joker{ --Kookoo
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and not (context.individual or context.repetition) then
            ...
            if not context.blueprint then
                if SMODS.pseudorandom_probability(card, 'kooks', card.ability.extra.chance, card.ability.extra.odds, 'kookoo_chance') then
...
SMODS.Joker{ --Ire {Doesn't do anything}
    key = "ire",
    config = {},
    pos = { x = 0, y = 0 },
    rarity = 2,
    ...
    calculate = function(self, card, context)
        if context.setting_blind and not context.blueprint then
            ...
            print(#destructable_jokers)
            local first_joker_to_destroy = #destructable_jokers > 0 and pseudorandom_element(destructable_jokers, pseudoseed('ireness')) or nil
            if first_joker_to_destroy and not (context.blueprint_card or card).getting_sliced then 
                first_joker_to_destroy.getting_sliced = true
                local second_joker_to_destroy = #destructable_jokers > 1 and pseudorandom_element(destructable_jokers, pseudoseed('ireness2')) or nil
                if second_joker_to_destroy and second_joker_to_destroy ~= first_joker_to_destroy and not (context.blueprint_card or card).getting_sliced then 
                    second_joker_to_destroy.getting_sliced = true
...
SMODS.Joker{ --Duk {Doesn't do anything}
    key = "duk",
    config = {},
    pos = { x = 0, y = 0 },
    rarity = 2,
...
SMODS.Joker{ --Pointy (dev)
    key = "pointy",
    config = { spades = 400, clubs = 35, hearts = 4, diamonds = 8},
    pos = { x = 0, y = 0 },
    rarity = 4,
    cost = 20,
    blueprint_compat = true,
    ...
    loc_vars = function(self, info_queue, card)
        return { vars = { card.ability.spades, card.ability.clubs, card.ability.hearts, card.ability.diamonds } }
    end
}
